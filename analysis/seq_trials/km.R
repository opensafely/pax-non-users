...
if(length(args)==0){
  ...
  df_input <- "output/data/data_seq_trials_monthly.feather"
  dir_output <- "output/seq_trials/descriptives/survival"
  exposure <- c("arm")
  subgroups <- NULL
  tstart <- "tstart"
  tend <- "tend"
  event_indicator <- "status_seq"
  weight <- NULL
  ...
  option_list <- list(
    make_option("--df_input", type = "character", default = "output/data/data_seq_trials_monthly.feather",
    ...
    make_option("--dir_output", type = "character", default = "output/seq_trials/descriptives/survival",
    ...
    make_option("--tend", type = "character", default = "tend",
    ...
    make_option("--event_indicator", type = "character", default = "status_seq",
...
for (subgroup_i in subgroups) {
  ...
  if(plot){
    ...
    ggsave(filename = fs::path(dir_output, paste0(file_name, ".png")), km_plot, width = 20, height = 30, units = "cm", bg = "white")
    ggsave(filename = fs::path(dir_output, paste0(file_name, "_red.png")), km_plot_red, width = 20, height = 30, units = "cm", bg = "white")
  }
}
